...
export const CATALOG = {
  ...
  ammeter: {
    category: 'meter',
    brief: '测量电流，必须串联在被测电路中，让电流从“+”接线柱流入。',
    detail: '电流表内阻极小，理想情况下相当于一段导线，因此绝不能直接并在电源或用电器两端——那相当于短路，会烧坏电流表。读数时先确认量程（0–0.6 A 或 0–3 A）再读数。本实验中电流表按理想处理：不计内阻，但保留了极性与量程的教学规则。'
  },
  voltmeter: {
    category: 'meter',
    brief: '测量电压，必须并联在被测电路两端。电压是两点之间的电势差。',
    detail: '电压表内阻极大，理想情况下相当于断路，因此可以直接并在电源两端测电源电压。若把电压表错误地串入电路，电路几乎不通，电压表读数接近电源电压。本实验中电压表按理想处理：内阻视为无穷大。'
  },
  galvanometer: {
    category: 'meter',
    brief: '检测微小电流的灵敏仪表，指针在刻度盘中央，可以显示电流方向。',
    detail: '灵敏电流计的零刻度在中央，电流从“+”流入时指针向右偏，反向时向左偏。它常用于判断有无电流以及电流方向，也是改装电流表、电压表的核心部件。'
  },
  diode: {
    category: 'semi',
...
export const CONCEPTS = {
  ...
  voltage: {
    ...
    detail: '电路中每个点都有一个“电势”，可以理解为“电的高度”。选定参考点（通常取电源负极）为 0 V 后，其他点的电势就有了确定数值；两点电势之差就是这两点间的电压。电源的作用类似水泵：在内部把正电荷从低电势“搬”到高电势，维持两极之间的电势差。用电器两端的电压越大，通过它的电流一般越大。电压的单位是伏特（V）。打开“电势”叠加层，可以直接看到电势沿电路的“高低起伏”。'
  },
  short: {
  ...
  water: {
    title: '水路类比', en: 'WATER CIRCUIT ANALOGY',
    brief: '水泵 ↔ 电源，水位差 ↔ 电压，水流 ↔ 电流，阀门/窄管 ↔ 电阻，水轮机 ↔ 用电器。水路面板的水位差与流量会与电路数值实时同步。',
    detail: '水泵把水从低处抽到高处，维持两处水位差；水位差驱动水管中的水流，推动水轮机做功。类似地，电源维持电势差，驱动电路中的电流，使用电器工作。串联水路中流量处处相等、水位分段下降；并联水路中各支路水位差相同、总流量等于各支路之和——与串并联电路的规律一一对应。这个类比并非巧合：层流管道中流量确实与压差成正比（泊肃叶定律），形式上就是“水路的欧姆定律”。但类比有边界：电子不是小水滴，导线中的自由电子本来就存在，电流也不是“电从电源流出来被用掉”。类比只用来建立直觉。'
  }
};
...
export const PRESETS = [
  ...
  },
  {
    ...
    components: [['student-supply', 80, 510, { voltage: 4.5 }], ['switch', 350, 250, {}], ['rheostat', 620, 250, { max: 50, x: 1 }], ['resistor', 950, 250, { resistance: 10 }], ['ammeter', 1260, 250, {}], ['voltmeter', 950, 520, {}]],
    wires: [[0, 'pos', 1, 'a'], [1, 'b', 2, 'a'], [2, 'c', 3, 'a'], [3, 'b', 4, 'plus'], [4, 'minus', 0, 'neg'], [5, 'plus', 3, 'a'], [5, 'minus', 3, 'b']]
  },
  {
    ...
    components: [['student-supply', 80, 510, { voltage: 6 }], ['switch', 350, 250, {}], ['rheostat', 620, 250, { max: 100, x: 1 }], ['resistor', 950, 250, { resistance: 5 }], ['ammeter', 1260, 250, { range: 3 }], ['voltmeter', 950, 520, {}]],
    wires: [[0, 'pos', 1, 'a'], [1, 'b', 2, 'a'], [2, 'c', 3, 'a'], [3, 'b', 4, 'plus'], [4, 'minus', 0, 'neg'], [5, 'plus', 3, 'a'], [5, 'minus', 3, 'b']]
  },
  {
    ...
    wires: [[0, 'pos', 1, 'a'], [1, 'b', 2, 'a'], [2, 'b', 3, 'plus'], [3, 'minus', 0, 'neg']]
  },
  {
  ...
  },
  {
    ...
    wires: [[0, 'pos', 1, 'a'], [1, 'b', 2, 'a'], [2, 'b', 0, 'neg'], [3, 'plus', 2, 'a'], [3, 'minus', 2, 'b']]
  },
  {
    ...
    wires: [[0, 'pos', 1, 'plus'], [1, 'minus', 2, 'a'], [2, 'b', 0, 'neg']]
  },
  {
    ...
    wires: [[0, 'pos', 1, 'minus'], [1, 'plus', 2, 'a'], [2, 'b', 0, 'neg']]
  },
  {

...
export function drawPlot(svg, records, targetLabel) {
  ...
  // 边框与坐标轴
  el('rect', { x: 0.5, y: 0.5, width: W - 1, height: H - 1, fill: 'none', stroke: 'rgba(70,105,135,.22)' }, svg);
  el('line', { x1: ml, y1: mt + ph, x2: ml + pw, y2: mt + ph, stroke: 'rgba(70,105,135,.55)', 'stroke-width': 1 }, svg);
  el('line', { x1: ml, y1: mt, x2: ml, y2: mt + ph, stroke: 'rgba(70,105,135,.55)', 'stroke-width': 1 }, svg);
  ...
  for (let k = 1; k <= 4; k++) {
    ...
    el('line', { x1: X(u), y1: mt + ph, x2: X(u), y2: mt + ph + 3, stroke: 'rgba(70,105,135,.55)' }, svg);
    const i = maxI * k / 4;
    text(svg, trim(i), { x: ml - 4, y: Y(i) + 3, class: 'sch-label', 'text-anchor': 'end', 'font-size': 8 });
    el('line', { x1: ml - 3, y1: Y(i), x2: ml, y2: Y(i), stroke: 'rgba(70,105,135,.55)' }, svg);
  }
  ...
  el('path', { d, fill: 'none', stroke: 'rgba(14,134,198,.75)', 'stroke-width': 1.6 }, svg);
  for (const p of sorted) {
    el('circle', { cx: X(p.u), cy: Y(p.i), r: 3, fill: '#0e86c6' }, svg);
  }
  if (targetLabel) {
